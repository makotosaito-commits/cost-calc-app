    ...
    return (
        <div className="space-y-4 md:space-y-2">
                            ...
                            >
                                {image ? (
                                    ...
                                    <div className="text-center text-muted-foreground transition-colors">
                                        <PhotoIcon className="h-10 w-10 md:h-7 md:w-7 mx-auto mb-2 md:mb-1" />
                        ...
                        <div className="mt-3 space-y-4 md:mt-0.5 md:space-y-2">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    className="text-lg md:text-sm font-semibold bg-transparent border-t-0 border-l-0 border-r-0 border-b border-border rounded-none focus-visible:ring-0 focus-visible:border-foreground transition-all px-1"
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <div className="relative group">
                                    ...
                                    <Input
                                        ...
                                        placeholder="例: 850"
                                        className="text-xl md:text-base font-semibold bg-transparent border-t-0 border-l-0 border-r-0 border-b border-border rounded-none focus-visible:ring-0 focus-visible:border-foreground transition-all pl-5 pr-1"
                                    />
                                </div>
                            </div>
                        </div>
            ...
            <Card className="bg-card border-border shadow-sm">
                <CardContent className="p-6 md:p-3 space-y-4 md:space-y-2">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">結果</p>
                    <div className={`rounded-xl border p-4 md:p-2 ${rateToneClass}`}>
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">原価率</p>
                                <p className="mt-1 text-4xl md:text-2xl font-black leading-none tabular-nums">
                                    {evaluated.displayRate ?? '—'}
                                    <span className="ml-1 text-lg md:text-sm font-bold">%</span>
                                </p>
                            </div>
                            <span className="text-[11px] md:text-[9px] font-medium tracking-wide rounded-full px-2 py-1 md:px-1.5 md:py-0.5 border border-current/30 bg-background/50">
                                {rateLabel}
                            </span>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3 md:gap-1.5">
                        <div className="bg-muted/40 rounded-xl p-4 md:p-2 border border-border">
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">粗利益</p>
                            <p className={`text-xl md:text-base font-semibold tabular-nums ${hasSalesPrice ? 'text-foreground' : 'text-muted-foreground'}`}>
                                {safeGrossProfit !== null ? safeGrossProfit.toLocaleString() : '—'} <span className="text-xs font-normal">円</span>
                            </p>
                        </div>
                        <div className="bg-muted/40 rounded-xl p-4 md:p-2 border border-border flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">原価合計</p>
                                <p className="text-xl md:text-base font-black text-foreground tabular-nums">{safeTotalCost !== null ? safeTotalCost.toLocaleString() : '—'} <span className="text-xs font-normal">円</span></p>
                            </div>
                            <div className="h-9 w-9 md:h-7 md:w-7 flex items-center justify-center bg-background border border-border rounded-lg text-muted-foreground">
                                <ReceiptIcon />
                            </div>
                        </div>
                    </div>
